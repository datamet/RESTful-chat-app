...
triggers.set(' agree ', () => res.like)
triggers.set(' cool ', () => res.like)
triggers.set(' nice ', () => res.like)
triggers.set(' right on ', () => res.like)
triggers.set(' to bad ', () => res.like)
triggers.set(' no worries ', () => res.like)
...
triggers.set(' please leave ', () => res.date)
triggers.set(' go away ', () => res.date)
triggers.set(' turn off ', () => res.date)
triggers.set(' kick bot ', () => res.date)
...
export default info => (msg, options) => {
    ...
    if (!msg) createRespons(res.inactive)
    const { sender, message } = msg
    addContext(info, sender, message)
    if (message) {
